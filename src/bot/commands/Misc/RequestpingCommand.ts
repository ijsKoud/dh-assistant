...
export default class RequestpingCommand extends Command {
	public async messageRun(message: GuildMessage) {
		...
		const embed = this.client.utils.embed().setTitle("🔔 Ping request").setDescription(`Requested by ${message.member.toString()}`);
		const components = new MessageActionRow().addComponents(
			new MessageButton().setStyle("SUCCESS").setEmoji(this.client.constants.emojis.greentick).setCustomId("pingrequest-accept"),
			new MessageButton().setStyle("DANGER").setEmoji(this.client.constants.emojis.redcross).setCustomId("pingrequest-decline")
		);
