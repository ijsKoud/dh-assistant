...
export default class QotdCommand extends Command {
	public async messageRun(message: GuildMessage) {
		...
		let msg: Message = await message.channel.send(
			base + "Please give me the QOTD including the user (<qotd> ~ <user>)"
		);
		...
		if (!qotd || !qotd.includes("~"))
			return msg.edit(base + "Prompt closed - invalid QOTD provided.");
		if (collector.content.match(/cancel/g))
			return msg.edit(base + "Prompt closed - cancellation request.");

		// FOTD
		msg = await message.channel.send(
			base + "Please give me the FOTD including the user (<qotd> ~ <user>)"
		);
		collector = (await this.client.utils.awaitMessages(msg, { filter }))?.first();
		if (!collector) return msg.edit(base + "Prompt closed - no response");
		const fotd = collector.content;
		if (!fotd || !fotd.includes("~"))
			return msg.edit(base + "Prompt closed - invalid FOTD provided.");
		if (collector.content.match(/cancel/g))
			return msg.edit(base + "Prompt closed - cancellation request.");

		// Source
		msg = await message.channel.send(
			base + "Please give me the Source of the FOTD (excluding the 'source:')"
		);
		collector = (await this.client.utils.awaitMessages(msg, { filter }))?.first();
		if (!collector) return msg.edit(base + "Prompt closed - no response");
		...
		const embed = this.client.utils
			...
			.setDescription(
				[
					...
					"FOTD",
					fotd,
					"_Don't forget to leave your responses in_\n<#702174849283522661>",
				].join("\n\n")
			);
