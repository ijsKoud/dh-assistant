...
export default class TictactoeCommand extends Command {
	public async messageRun(message: Message, args: Command.Args) {
		...
		if (!member || member.id === message.author.id) return message.reply(">>> 🔎 | I was unable to find the user.");

		if (member.presence?.status === "offline")
			return message.reply(">>> 💤 | Sorry, this user is offline. You can only start a game once the user is online again!");
