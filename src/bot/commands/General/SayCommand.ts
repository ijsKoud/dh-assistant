...
export default class SayCommand extends Command {
	public async run(message: Message, args: Args) {
		...
		if (!msg)
			return message.reply(`>>> ${client.constants.emojis.redcross} | No message provided.`);

		// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
		if (!channel.permissionsFor(message.member!).has(["SEND_MESSAGES", "VIEW_CHANNEL"]))
			return message.reply(
				`>>> ${client.constants.emojis.redcross} | You aren't allowed to talk here, you can only echo to channels you can talk in!`
			);

		// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
		if (!channel.permissionsFor(message.guild!.me!).has(["SEND_MESSAGES", "VIEW_CHANNEL"]))
			return message.reply(
				`>>> ${client.constants.emojis.redcross} | I am not allowed to talk in this channel.`
			);
		...
		await message.react(client.constants.emojis.greentick).catch(() => void 0);
	}
}
