	...
	public async messageRun(message: Message, args: Command.Args) {
		...
		if (!id)
			return message.reply(
				`>>> ${this.client.constants.emojis.redcross} | No user/guild id provided.`
			);
		if (!this.client.blacklistManager.blacklisted.includes(id))
			return message.reply(
				`>>> ${this.client.constants.emojis.redcross} | User/guild is already whitelisted.`
			);

		const data = (await this.client.utils.fetchUser(id)) || (await this.client.guilds.fetch(id));
		if (!data)
			return message.reply(`>>> ${this.client.constants.emojis.redcross} | No user/guild found.`);
