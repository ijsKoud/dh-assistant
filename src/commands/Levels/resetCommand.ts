...
export default class resetCommand extends Command {
	...
	async exec(message: Message, { id }: { id: string }) {
		...
		const emojis = [this.client.emoji.greentick, this.client.emoji.redcross];
		...
		if (!res || res.first()?.emoji.toString() === this.client.emoji.redcross) return msg.delete();

		await Level.findOneAndDelete({ userId: user.id, guildId: message.guild.id });
		await msg.edit("Successfully deleted their xp + level.");
		await msg.reactions.removeAll();

		this.client.log(
			"INFO",
			`User XP (${member.user.tag} / ${member.user.id}) reset by by ${message.author.tag} / ${message.author.id}`
		);
	}
}
