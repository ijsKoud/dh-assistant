...
import { iWarn } from "../../models/interfaces";
		...
		if (member && id) {
			...
			if (!data?.length)
				return message.util.send(
					this.client.responses.missingArg(`No warnings found for **${member.user.tag}**.`)
				);
			...
			if (!data.length)
				return message.channel.send(`>>> 🥳 | No warnings for this server found, yay!`);

			const tempCache = new Collection<string, number>();
			data.forEach((w) => tempCache.set(w.userId, (tempCache.get(w.userId) || 0) + 1));
